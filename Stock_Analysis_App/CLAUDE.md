...
- **No of Shares & Total Value on Trades** — `quantity` field renamed to "No of Shares" in UI; `totalValue` (= entryPrice × quantity) computed server-side and returned in TradeResponse; live Total Value preview in trade form; Shares + Total Val. columns in TradeTable; detail page shows both fields; form layout improved with 2-col No of Shares + Setup Type row and 4-col R/R + Total Value info box
...
- [x] **No of Shares & Total Value on Trades** — "Quantity" relabelled "No of Shares" in UI; `totalValue` (= entryPrice × quantity) added to `TradeResponse` as a computed field; live preview in trade form as 4th column of R/R box; Shares + Total Val. columns added to TradeTable; detail page shows both; form layout improved with 2-col No of Shares + Setup Type row (2026-06-01) — see `docs/2026-06-01-no-of-shares-total-value.md`
